...
import org.sigil.BuilderConfirm
...
internal fun CameraPanel(close: () -> Unit, use: suspend (ByteArray) -> Unit) {
        ...
        if (granted && photo == null) {
            ...
            future.addListener({
                if (!disposed) try {
                    ...
                    val camera=provider!!.bindToLifecycle(lifecycle, if (front) CameraSelector.DEFAULT_FRONT_CAMERA else CameraSelector.DEFAULT_BACK_CAMERA, feed!!, capture)
    ...
    Column(Modifier.fillMaxSize()) {
        org.sigil.CameraViewfinder(Modifier.weight(1f),photo=photo!=null,ready=ready,busy=taking,issue=issue,close=close,capture=takePhoto,
            retake={photo?.let {bitmap->photo=null;bitmap.recycle()}},flip={front=!front},flash=if(hasFlash)(flash to {flash=!flash}) else null) {
            val bitmap=photo
            ...
            else SigilButton({permission.launch(Manifest.permission.CAMERA)},Modifier.align(Alignment.Center)){Text("Allow camera")}
        }
        photo?.let {bitmap->
                BuilderConfirm("Attach photo",!taking) { taking = true; encoding = bitmap; scope.launch(start = CoroutineStart.UNDISPATCHED) {
