...
//! a unix socket speaking JSON lines. The protocol logic lives in the library.

use sigil_engine::{ipc, paths, shm};
...
fn main() -> anyhow::Result<()> {
    ...
    match cli.cmd.unwrap_or(Cmd::Daemon {
        ...
        Cmd::Cli {
            ...
            rt.block_on(ipc::client::run(
                socket.unwrap_or_else(paths::socket_path),
                req,
                params,
                follow,
            ))
        ...
        Cmd::Shmtest {
            ...
            seconds,
            grow_at,
        } => shmtest(width, height, fps, seconds, grow_at),
...
fn doctor() -> anyhow::Result<()> {
    ...
    println!(
        "account: {}",
        if sigil_engine::sigil::has_account() {
            "saved"
        } else {
            "none"
        }
    );
    println!("calls:   not yet (Phase 7)");
    Ok(())
}
...
fn shmtest(w: u32, h: u32, fps: u32, seconds: u32, grow_at: u32) -> anyhow::Result<()> {
    ...
    println!(
        "writing {} ({w}x{h} @{fps} for {seconds}s)",
        wr.path().display()
    );
    ...
    for f in 0..frames {
        ...
        if let Some(d) = t.checked_duration_since(std::time::Instant::now()) {
            std::thread::sleep(d);
        }
    }
...
fn shmdump(key: &str, out: &std::path::Path) -> anyhow::Result<()> {
    let path = if key.contains('/') {
        ...
        paths::shm_dir().join(format!("video-{key}.shm"))
    };
    ...
    for y in 0..h as usize {
        ...
        for x in 0..w as usize {
            img.put_pixel(
                x as u32,
                y as u32,
                image::Rgba([row[x * 4], row[x * 4 + 1], row[x * 4 + 2], 255]),
            );
        }
    }
    img.save(out)?;
