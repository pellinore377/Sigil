...
| `account.recover` | `username`, `password`, `code?`, `envoy?` | with a code: the classic restore; without one, the server's escrow opened with the password, handed out only to a sign-in that holds the name (run `account.oidcStart` first); then the backup replays and `status` becomes `loggedIn` |
| `account.oidcStart` | `server` (hostname), `issuer`, `clientId` | starts the sign-in at the server's identity provider: PKCE, a loopback listener for the redirect; returns `{url}` for the app to open in the browser. Ends in an `oidc.state` event: `done` with `name` (the provider's username, a suggestion) and `held` (the localpart this login already holds here, empty on a first visit), or `failed` with `error`. The ID token stays in memory and `account.create` with an empty `invite` presents it as the gate |
