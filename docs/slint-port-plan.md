...
| **SSO** | `account.oidcStart{server, issuer, clientId}`, then `oidc.state` events, then `account.create` or `account.recover` | When the server gates names with OIDC the probe opens the browser at once (PKCE, loopback redirect) and the doors wait for it. `oidc.state: done` carries the provider's name suggestion and `held`, the name that login already holds on this server. Nothing held: the **name** door (suggestion prefilled), then the **recovery password** door (plain words about what the password is for, no printed code), then Home. A held name: the **welcome back** door, the recovery password alone, or "Link this device instead". Every path is gated by that token |
| **Recovery code** | `recovery.code` | Only on servers with `recovery = "code"`: shown once after account creation with a password (the create reply says `escrowed: false`), and from Settings. With `recovery = "escrow"` the key sits sealed on the server under the password and nothing is printed; `recovery.status` says `escrow: true` and Settings hides the code row |
