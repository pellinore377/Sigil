...
`sync_backend_due_online` includes the durable push lane. Platform adapters supply token/endpoint changes and push bytes to the native APIs, then schedule work using returned deadlines. Hints never bypass mailbox authentication, backoff or local decryption. FCM wake pushes use high-priority data messages, since each precedes a visible message notification and Doze would otherwise hold them for minutes; ownership challenges stay normal priority. Senders mark receipts, non-invitation call controls and key shares with the `sigil-silent: 1` header and the server stores them without a push job, so high-priority pushes always correspond to something a person will see and providers have no reason to demote them. Distributor selection, Android callbacks/permissions, notification construction and testing with two distributors remain platform acceptance.
