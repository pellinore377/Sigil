...
## Backend increment: canonical inline text (native schema 50)

The shared `sigil-text` Rust package now owns bounded source parsing, canonical
NFC text, grapheme ranges, resolved effect descriptors, redaction and generated
HTML fallback. Unicode normalization and extended grapheme segmentation are
both pinned to **Unicode 17.0.0** through `unicode-normalization` 0.1.25 and
`unicode-segmentation` 1.13.3. CommonMark parsing uses `pulldown-cmark` 0.13.4.
MIT-selected dependency notices are in `licenses/Text-ThirdParty.txt` and the
existing server notices. This is an initial implementation, not completion of
this specification or item 6.

Supported inline behavior includes the named colors/gradients, emphasis,
decoration, sizes, animation names, highlight, monospace, spoiler/scratch metadata,
`||spoiler||`, escapes, line-scoped incomplete spans and code exclusion. Parsing
preserves the original CommonMark structure; removing a modifier does not turn
its content into a new heading/list. Entity terminators and link metadata cannot
terminate a text modifier. Raw HTML is escaped, image syntax supplies alt text,
and only explicit HTTP(S)/mailto links can remain actionable. No parsing step
performs networking. Full block-level rich HTML, bare-URL detection, structured
constructs, mentions/contact resolution and animation parameters remain open.

`Text::from_runs` is the graphical construction boundary; `redact_range` uses
canonical grapheme indices and removes selected content before producing any
serialized representation. Recognized typed redaction removes the enclosing
content, including nested Markdown/code/link data. Redacted link labels also
lose their link destinations. Code containing a literal `redact::` opener stays
literal. Redaction has no retained secret/reveal field. Styles that touch part of
a grapheme expand to that complete grapheme; later source pieces resolve exclusive
effects. Empty runs add no styling.

The version-1 JSON body contains plaintext, generated HTML and `sigil` metadata
with the Unicode version and ordered `{kind,value}` effect descriptors. Color
values are canonical theme names; gradients carry ordered `stops`. Ordinary
highlight without a color uses the theme background. Incoming data must match
its regenerated HTML and canonical encoding; unknown fields/versions, invalid or
overlapping ranges, duplicate effects and unsafe link schemes are rejected.
The source is never transmitted alongside its redacted representations. These
checks do not prevent a malicious author from sending arbitrary ordinary text.

Default/hard bounds are 32 KiB source/body, 1,024 resolved spans, 32 nesting levels,
4,096 intermediate pieces/edits, nine gradient stops, 2,048 link bytes, 4,096 bytes
per grapheme and 60 KiB canonical wire data. Callers may lower source/body/span
limits. Overflow is an error, never a fallback containing the unredacted source.
Unknown inline tokens remain literal; unimplemented structured constructs do not
create interactive state.

Native schema 50 fences older clients that cannot interpret rich event kinds
5/6 or recovery content kind 3. Direct/group delivery, durable retry and recovery
use the existing authenticated paths. New outgoing rich handoffs check archive
tombstones; this does not erase old message journals, recall delivered content or
complete queued-packet retention. The probe's obsolete substring redactor has
been removed and delegates to this parser; its local error guard was updated.
Production layout and platform interaction acceptance remain paused.

Nine domain tests cover source/graphical equivalence, descriptor shape, Unicode,
redaction, escapes, malformed/forged data and 4,096 deterministic mixed-input
cases. Three native tests cover actual HTTPS delivery, transaction failures,
restart/fresh-device recovery, typed group fan-out, deletion barriers and an
authenticated malformed rich payload leaving the receiver ratchet uncommitted.
Protocol and recovery codec tests preserve the distinction from literal text.
These are implementation checks, not independent security review or full Unicode
conformance-vector acceptance.

During implementation, regression tests reproduced two source-parser mistakes:
reparsing after modifier removal changed Markdown structure, and source entity/
URL punctuation could prematurely terminate modifiers. Both are fixed. Test
setup also initially retried an already-completed upload and incorrectly expected
a direct-session API to retry an initial envelope; the tests now use each API's
actual contract. The initial struct-shaped effect serialization was replaced by
the specification's ordered descriptors before acceptance.

Specification references: [Unicode normalization](https://www.unicode.org/reports/tr15/)
and [Unicode grapheme segmentation](https://www.unicode.org/reports/tr29/tr29-47.html).

Acceptance after descriptor alignment: **481 workspace tests pass, nine skipped**;
optimized client **247 pass, four skipped** (190 library tests). With the unchanged
explicit bulk-cache case, **486 distinct tests** pass. Warnings-denied Clippy,
Wasm core compilation and rebuilt container restart/restore checks pass. Exact
logs and remaining gates are in [BackendProgress.md](BackendProgress.md). The
advisory database reports no listed vulnerabilities and retains the earlier
unmaintained macro warning; this does not audit the new parser.

## Backend increment: standalone structured cards (native schema 51)

One canonical `Card` model now represents notes, standard/task/recurring checklists
and polls. `parse_card` and graphical callers use the same model and deterministic
item identifiers. This increment implements creation, validation and delivery;
it does **not** yet implement interactive state transitions or finish item 6.

The syntax boundary currently accepts a standalone construct beginning at the
start of the message. Mixed ordinary text and multiple structured blocks remain
noninteractive text. Checklist markers are classified before Markdown; inline
labels share the existing normalization, styling and redaction parser. Code,
escaped semicolons, entities and link metadata do not terminate a block. Missing
termination yields an incomplete hint; invalid structure stays noninteractive.
Recurring syntax requires an explicitly supplied timezone. Poll option order is
independent, repeated categories resolve last-wins with a warning, and default
selection/disclosure remain single/open. All transmitted labels and generated
fallbacks contain the redacted representation only.

Cards carry the canonical account reference, creation timestamp, message ID and
bounded labels/items. The account reference includes the authenticated server
and account, excluding device keys and IDs. Direct and group acceptance compare
the claimed creator, timestamp and message ID against the authenticated event
before committing receiver state. An authenticated member still cannot claim a
different creator. This is attribution, not a signature transferable outside the
verified conversation. Recovery preserves the already accepted canonical card;
an imported owner-encrypted archive remains an explicitly trusted history source.

Default/hard limits are 256 checklist items, 64 poll options, 512-byte titles,
2,048-byte item labels, and the existing aggregate 32-KiB body/1,024-span/60-KiB
wire limits. Duplicate/noncanonical identifiers, unknown fields, forged fallback
text/HTML and invalid recurrence metadata fail. `Content::card()` is distinct
from the inline-only `rich()` accessor; both use rich event kinds 5/6 and recovery
kind 3. Native schema 51 fences clients without this structured decoder.

Recurrence calculations use pinned Jiff 0.2.35 with bundled `jiff-tzdb` 0.1.8,
[IANA 2026c](https://www.iana.org/time-zones). No device timezone database, local
zone lookup or network request participates. The original date anchors weekly,
monthly and yearly cycles; month-end clamping snaps back rather than drifting.
Rules store the timezone and database version; unknown/mismatched versions fail.
Timezone-data upgrade/backward compatibility remains work to define before broad
interoperability. Permissive dependency notices are in `licenses/Text-ThirdParty.txt`.

**Calendar policy refinement:** 00:01 is the target local reset time. If a clock
transition removes it, compatible resolution shifts it forward by the gap; if
it repeats, the earlier instant wins. Some zones have midnight transitions, so
an unconditional 00:01 guarantee is impossible. Tests cover those cases, normal
DST, leap/month-end recovery, long offline catch-up, overflow and the bundled
Alberta rule. This calculates instants; automatic reset actions, editing authority
and notifications are not yet connected.

Four structured-domain tests and three native tests cover canonical builder
agreement, redaction, incomplete/code cases, poll options, forged metadata,
restart/exact retry, encrypted history and direct/group creator forgery. Three
calendar tests cover the policy above. Rejected authenticated cards leave the
receiver ratchet unchanged; group acceptance succeeds after skipping the rejected
counter. The recovery codec also round-trips a structured card.

Acceptance: **491 workspace tests pass, nine skipped**; optimized client **250
pass, four skipped** (193 library tests). With the previously accepted unchanged
bulk-cache case, **496 distinct tests** pass. Warnings-denied Clippy, Wasm core
compilation, Docker rebuild and container restart/restore pass. The advisory scan
reports zero listed vulnerabilities and the existing unmaintained macro warning.
This is implementation acceptance, not an independent audit.

Open: action permissions/conflict resolution, task undo and attribution updates,
recurring reset/edit scheduling, poll voting/closure, structural edits, mixed
blocks, other Part II constructs, full block-level HTML and platform rendering.

## Backend increment: durable checklist changes and ballots (native schema 52)

Standard checklist check/uncheck actions and poll vote/withdrawal actions now
travel as canonical rich events. The authenticated account is the action actor;
a device cannot claim another voter or completing user. Native direct/group
queue APIs validate local actions, and delivery commits the action journal,
derived state, message/history and ratchet checkpoint together. Task undo,
recurring resets, structural edits, delegated edit permissions and poll closure
remain unimplemented; unsupported actions cannot mutate those constructs.

Each action references the creation card's ID, creator and canonical digest,
preventing cross-card substitution or merging different creation payloads. The
transport message ID is a domain-separated hash of the exact action. Its optional
predecessor must be from the same register: a checklist item, or the acting
account's ballot. There is no sender-provided logical clock. Validated predecessor
depth determines ordering; concurrent siblings resolve by canonical account and
action ID. Different items/voters remain independent, and another device of the
same account edits the same ballot. This is deterministic conflict resolution,
not a claim of real-time ordering, fairness or a secret-ballot protocol.

Authenticated, well-formed actions arriving before their card/predecessor remain
pending. Once dependencies arrive, invalid options/targets or incompatible
ancestors become rejected journal entries without changing the card. Origin or
canonical-wire failures reject the message before ratchet commitment. Those two
outcomes are intentionally distinct. Exact retries do not count another vote.

Encrypted local tables retain the creation, causal action nodes, per-register
winners and aggregate counts. Poll reads do not scan the entire voting history.
A durable work queue visits at most 64 dependent rows/work units per offline pass;
newly committed actions may resolve immediately. Indexed card/parent queues and
persisted cursors avoid repeatedly scanning unrelated missing dependencies.
Large branches rotate through the queue. Ordinary sync invokes an offline pass;
platforms can also call `advance_structured_actions`. Card reads use one database
snapshot. Action history is bounded by the existing database storage budget,
not a fixed lifetime operation count.

`card_state` returns resolved checks and the viewing account's choices. Closed
poll counts are withheld until that account has a nonempty ballot, including
following withdrawal. Participants still possess decrypted action data; this is
application visibility, not cryptographic result secrecy.

Live direct/group capture and explicitly trusted recovery imports rebuild the
same derived state. Schema 51 migration covers authenticated retained direct and
group content, including cards without recovery configured. Intact creation and
causal action history recover on a fresh reauthorized device without importing
identity/prekey/session/push state. Missing/deleted ancestors remain pending;
causal snapshots/compaction and full action-journal retention are still open.
Deleting an action from the history archive does not currently erase its live
causal node or undo its resulting state. No complete retention claim is made.

A deleted creation record marks its derived source obsolete; deleting all sources
hides the card and blocks new outgoing actions, including queued handoff before
packet commitment. A tombstone preceding the original delivery or migration
cannot recreate a live source. Already frozen network packets and other members'
copies are not recalled, and retained local journals are not physically erased.

Three domain tests cover causal/register rules, canonical IDs and origin binding.
Eight native tests cover delivery order, a 150-action reversed chain, concurrent
writers, exact retries, rejected ancestors, encrypted direct/group transaction
failures, group actor forgery, fresh recovery, migration and deletion barriers.
Test setup corrections included using the production immediate-transaction
contract, acknowledging an earlier mailbox item, allowing a newly selected
session to be absent before receipt, and starting a new archive entry at revision
one. These were fixture mistakes, not discovered backend vulnerabilities.

Acceptance: **502 workspace tests pass, nine skipped**; optimized client **258
pass, four skipped** (201 library). With the unchanged explicit bulk-cache check,
**507 distinct tests** pass. Warnings-denied Clippy, final text-domain tests, Wasm
core compilation and rebuilt container restart/restore pass. The prior dependency
advisory result is unchanged; no dependencies were added for actions. No whole
additional numbered backend item is complete.

## Backend increment: task completion and undo (native schema 53)

Task lists use canonical `Complete` and `Undo` actions rather than ordinary checklist toggles. Each completion has a content-derived operation ID and authenticated account attribution. Undo names that exact completion as its predecessor, uses the same account and item, and declares a timestamp from completion time through strictly before completion time plus 30 seconds. Another account, another item, a missing predecessor, or a timestamp outside that interval is rejected. Offline receipt does not itself expire an otherwise valid declared undo. As the plan requires, these timestamps express application behavior, not proof of a deadline against a modified client.

Concurrent completions are an attributed set. Undo removes only the named completion; it cannot erase another user's concurrent completion. An item remains completed while any completion remains active. Multiple completions by the same account on different devices are likewise separate unless their canonical action bytes are identical. Active attribution is paginated in at most 64 entries, with a sealed aggregate count; no fixed lifetime completion limit replaces the native storage budget. Pages are individually consistent snapshots, not a promise that a multi-page scan ignores concurrent changes. Items initially checked in the creation card are attributed to its creator and remain initially completed; no fabricated undo operation is created for them.

`CardState.tasks` supplies task-specific state separately from ordinary `checks`. `task_completions` exposes active operation/account/timestamp attribution and the exclusive `undo_until` instant. Native schema 53 adds encrypted counts and authenticated completion indexes. Updates share the existing message/history/ratchet transaction, including group delivery and trusted history-only recovery. Creation tombstones continue to block state access and new outgoing actions. Undo evidence is retained; this does not implement full action-history compaction or physical erasure.

Four new native tests and one domain test pass, covering reversed delivery, exact author/window rules, concurrent writers, rollback, restart, 130 attributed completions across pages, direct encrypted completion/undo and schema-52 migration. Existing group and fresh-device recovery acceptance now also run task scenarios, including unchanged ratchets on failed writes and no restored live keys. Workspace: **543 passed, nine skipped**. Optimized client: **266 passed, four skipped** (209 library and 57 integration); **548 distinct** including the unchanged explicit 1 GiB cache case. Clippy and Wasm compilation pass. Logs: `/tmp/sigil-task-domain-tests.log`, `/tmp/sigil-task-structured-tests.log`, `/tmp/sigil-task-group-tests.log`, `/tmp/sigil-schema53-workspace.log`, `/tmp/sigil-schema53-release.log`, `/tmp/sigil-schema53-clippy.log`, `/tmp/sigil-schema53-wasm.log`. No new dependency or cryptographic primitive was introduced. Recurring reset/edit behavior, poll closure, structural edits/delegation and broader SigilText constructs remain open; item 6 is not complete.

## Backend increment: recurring checklist periods (native schema 54)

`RecurringCheck` names an item and the exact recurrence-period start. Its declared timestamp must belong to that period; it has no manually unchecked form and no predecessor chain. A separate register per item/period prevents a delayed old check from affecting a later cycle. Concurrent checks within one period retain the existing deterministic actor/operation ordering. One-off items only accept checks in the creation's initial period; persistent `-r-` items remain available in later periods.

`Recurrence::period_at` derives the active `[start,end)` interval from the original calendar anchor and bundled timezone rules in at most three candidate calculations. `recurring_state` uses the caller's trusted current time: persistent items uncheck at the boundary and one-offs disappear from the current list. Original creation/history records remain retained; projection is not physical deletion. Years of missed resets require no reset-packet replay or database sweep. The returned end instant supports later platform scheduling. Static `CardState.checks` does not pretend to be a clock-resolved recurring view.

Two new domain tests and one native test cover exact boundaries, daylight-saving transitions, month-end catch-up, one-off removal, delayed/reordered checks, restart and read-only ten-year catch-up. Existing direct/group/recovery scenarios now also exercise recurring checks, including authenticated origin, transaction failure and fresh-device recovery without live keys. Workspace **546 passed, nine skipped**; optimized client **267 passed, four skipped** (210 library); **551 distinct** with the unchanged explicit cache case. Clippy, Wasm and rebuilt container restart/restore pass. Logs: `/tmp/sigil-recurring-domain-tests.log`, `/tmp/sigil-recurring-native-tests.log`, `/tmp/sigil-recurring-group-tests.log`, `/tmp/sigil-schema54-workspace.log`, `/tmp/sigil-schema54-release.log`, `/tmp/sigil-schema54-clippy.log`, `/tmp/sigil-schema54-wasm.log`, `/tmp/sigil-schema54-container-build.log`, `/tmp/sigil-schema54-container.log`.

The optimized run exposed a synthetic task test advancing its transport clock one second ahead of the HTTPS server, exceeding the server's maximum seven-day expiry. Completion and undo now use the same captured transport second; the full optimized rerun passes. The failure is retained at `/tmp/sigil-schema54-release-fixture-before.log`. Production expiry rules were not changed.

Recurrence settings remain fixed by the creation card; authorized edits/delegation and structural list changes remain open. Platform wakeups and notifications are not implemented here. Sender timestamps remain application claims, and the projection API requires a trusted caller clock. Poll closure and broader SigilText constructs remain unfinished; item 6 is not complete.
