...
pub fn rewind(db: &rusqlite::Connection, version: u32) {
    if version < 74 {
        db.execute_batch(
            "DROP TABLE IF EXISTS mobile_profiles; DROP TABLE IF EXISTS mobile_photo_upload;",
        )
        .unwrap();
    }
    if version < 73 {
        db.execute_batch(
            "DROP TABLE IF EXISTS mobile_contacts; DROP TABLE IF EXISTS mobile_contact_poll;",
        )
        .unwrap();
    }
    if version < 72 {
