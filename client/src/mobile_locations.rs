...
impl ClientStore {
    pub(super) fn mobile_location_queue(
        &mut self,
        conversation: Id,
        action: CardAction,
    ) -> Result<(), Error> {
        let peer = if conversation == self.mobile_conversation("self")? {
            ...
            .mobile_peers()?
            .into_iter()
            .find(|peer| self.direct_conversation(peer.id).ok() == Some(conversation))
        {
        ...
        self.mobile_action(
            &peer,
            &transport::hex(&action.id().map_err(|_| Error::InvalidEvent)?),
            action.created_at,
            Action::Post {
    ...
    pub(super) fn mobile_location_work(
        &mut self,
        after: Option<Id>,
        point: Option<Point>,
        stop: bool,
        ...
        if let Some(point) = &point {
            ...
            if stop || point.sampled_at > now || now.saturating_sub(point.sampled_at) > 60 {
        ...
        let mut active = 0;
        let mut until = None;
        let mut queue = |store: &mut Self, conversation, action| match store
            .mobile_location_queue(conversation, action)
        {
            Ok(()) => queued += 1,
            Err(error) => issue = Some(error_message(&error)),
        };
        for (conversation, action) in batch.stops {
            queue(self, conversation, action);
        }
            ...
            if stop {
                ...
                if let Some(point) = &point {
                    if point.sampled_at >= job.next_sample_at {
                        let action = self.update_location(
                            job.conversation,
                            job.reference,
                            point.clone(),
                            now,
        ...
        Ok(
            json!({"active":active,"until":until,"queued":queued,"issue":issue,"next":batch.next.map(|id| transport::hex(&id))}),
        )
    }
}
