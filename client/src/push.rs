...
impl ClientStore {
    ...
    pub fn set_browser_push(&mut self, target: Target, now: u64) -> Result<(), Error> {
        ...
        let (mut state, before) = read(&tx, &self.key, &scope)?;
        time(&state, now)?;
        if matches!(state.preference, Preference::Browser { .. })
            && state.preference.target()?.as_ref() == Some(&target)
        {
            return Ok(());
        }
        state.preference = Preference::Browser {
            endpoint: endpoint.clone(),
            public_key: public_key.clone(),
            auth: Zeroizing::new(auth_secret.clone()),
    ...
    pub fn receive_browser_push(
        ...
        let (mut state, before) = read(&tx, &self.key, &scope)?;
        time(&state, now)?;
        if !matches!(&state.preference, Preference::Browser { endpoint:current, .. } if current == endpoint)
        {
