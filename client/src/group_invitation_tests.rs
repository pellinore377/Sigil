...
fn linked_device_retains_a_failed_stop_for_another_devices_live_location() {
    use sigil_protocol::{
        conversation::{Action as MessageAction, Body},
        text::{
            action::Reference,
    ...
    let card = alice
        .location_card(
            [214; 32],
            crate::structured::LocationKind::Live(Duration::Hour),
            Point {
            ...
            Text::plain("Synthetic share", Default::default()).unwrap(),
            now,
        )
        .unwrap();
    let reference = Reference::of(&card).unwrap();
    let operation = joining
    ...
    let status: serde_json::Value =
        serde_json::from_str(&joining.mobile_command(r#"{"command":"location_work"}"#)).unwrap();
