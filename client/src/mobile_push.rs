...
impl ClientStore {
    pub(super) fn mobile_browser_push(
        &mut self,
        action: &str,
        ...
        match action {
            "providers" => {
                return serde_json::to_value(self.connected_client()?.push_providers()?)
                    .map_err(|_| Error::InvalidEvent)
            }
            "register" => {
                let target = target.ok_or(Error::InvalidEvent)?;
                let providers = self.connected_client()?.push_providers()?;
                if !matches!(&target,sigil_protocol::push::Target::UnifiedPush{vapid_key,..} if providers.unified_push && providers.vapid_public_key.as_ref()==Some(vapid_key))
                {
            ...
            "receive" => {
                ...
                let hint =
                    self.receive_browser_push(endpoint.ok_or(Error::InvalidEvent)?, &bytes, now)?;
