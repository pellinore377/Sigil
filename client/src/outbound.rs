...
impl ClientStore {
    ...
    pub fn resume_outbound_online(&mut self, now: u64) -> Result<Vec<OutboundAttempt>, Error> {
        ...
        let ids: Vec<Vec<u8>> = self.db.prepare(
            "SELECT DISTINCT o.session FROM outbox o JOIN sessions s ON s.id=o.session WHERE o.packet IS NOT NULL AND o.session>?1 AND s.peer IS NOT NULL AND s.suite=2 AND s.retired=0 ORDER BY o.session LIMIT 16"
