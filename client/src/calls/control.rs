...
pub(crate) fn validate_receipt(
    ...
    let known = crate::peers::verified(db, key, peer)?;
...
pub(crate) fn install(
    ...
    let known = crate::peers::verified(tx, key, peer_id)?;
    ...
    let mut record = match load(tx, key, &wire.call) {
        Ok(record) => record,
        Err(Error::NotFound) => {
            let Body::Invite(state) = &wire.body else {
            ...
            let creator = state
                .participants
                .iter()
                .find(|p| p.member.key == state.roster.roster.owner)
    ...
    if !matches!(wire.body, Body::Invite(_)) && record.state.roster.roster.expires != wire.expires {
    ...
    match wire.body {
        Body::Invite(state) | Body::State(state) => {
