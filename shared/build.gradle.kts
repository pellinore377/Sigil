...
tasks.named("wasmJsBrowserDistribution") {
    doLast {
        ...
        destination.resolve("sigil-media-worker.mjs").writeText("import init, { media_worker_start } from './sigil_browser.js'; self.onmessage = async ({data}) => { self.onmessage = null; await init({module_or_path:data.module}); media_worker_start(data.port, data.gate); };\n")
        destination.resolve("sigil-passkey.mjs").writeText("import init, { passkey_window } from './sigil_browser.js'; await init(); await passkey_window();\n")
