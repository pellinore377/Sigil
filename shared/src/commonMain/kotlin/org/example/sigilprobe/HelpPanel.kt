...
internal fun HelpPanel(enabled:Boolean,back:()->Unit,initialQuery:String?=null,send:(String)->Unit) {
    ...
    var detail by remember {mutableStateOf(0.dp)}
    // A topic sizes the panel to its whole content instead of scrolling inside a fixed frame.
    val natural=if(selected==null || detail==0.dp)420.dp else header+detail+16.dp
    SideEffect {preferred?.invoke(natural)}
    ...
    Column(Modifier.fillMaxSize().padding(start=8.dp,end=8.dp,top=8.dp),verticalArrangement=Arrangement.spacedBy(8.dp)) {
        Row(Modifier.onSizeChanged {header=with(density){it.height.toDp()}},verticalAlignment=Alignment.CenterVertically) {
        ...
        AnimatedContent(selected,transitionSpec={
            ...
            } else Column(Modifier.fillMaxWidth().verticalScroll(rememberScrollState()).wrapContentHeight(unbounded=true)
                .onSizeChanged {detail=with(density){it.height.toDp()}},verticalArrangement=Arrangement.spacedBy(12.dp)) {
