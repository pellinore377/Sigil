...
internal fun ComposerPanel(draft: TextFieldState, analyze: (String) -> String, enabled: Boolean, notes: Boolean, command: Command, peer: String, voice: VoiceState, sent: Long, sentText: String?, requestContact: (() -> Unit)? = null, attachments: List<Transfer> = emptyList(), editingCaption: Boolean = false, attachmentTarget: Map<String, Any?> = mapOf("peer" to peer), send: (String, Boolean, String?) -> Unit) {
    ...
    val expandedHeight = if (panel.isNotEmpty()) minOf(if(panel in createItems.map {it.first} || panel in listOf("Code block","Create"))maxOf(keyboardHeight,420.dp) else if(LocalWideLayout.current && panel=="Attachments")256.dp else keyboardHeight,available) else 0.dp
    ...
    LaunchedEffect(attachmentDrafts.map {it.request to it.phase}) {if(attachmentDrafts.any {it.phase!="Staging"} && panel in listOf("Attachments","Voice"))change("")}
    ...
    LaunchedEffect(stagedSource,previewer) {if(stagedSource.isNotEmpty())stagedPreview=kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.Default){previewer?.invoke(stagedSource)}}
    ...
    Surface(shape = RoundedCornerShape(topStart = 24.dp, topEnd = 24.dp), color = if (LocalFooterHost.current == null) MaterialTheme.colorScheme.surface else androidx.compose.ui.graphics.Color.Transparent) {
        Column {
            Box(Modifier.fillMaxWidth().height(panelHeight).testTag("composer-panel")) {
                CompositionLocalProvider(LocalBuilderAction provides "Add to message") {
                AnimatedContent(panel, transitionSpec = {
                    ...
                    when (shown) {
                        ...
                        "Randomizer" -> builders.SaveableStateProvider("$peer:$shown") {
                            RandomizerBuilder(enabled,{change("Create")}) {source->stage(shown,source)}
                        }
                        ...
                        "Translation","Definition","Weather","Contact" -> ServiceBuilder(shown,{change("Create")},{command("service",mapOf("action" to "discard","request" to it))}) {query,preview,contact->stagedQuery?.takeIf {it!=query}?.let {command("service",mapOf("action" to "discard","request" to it))};stagedKind=shown;stagedSource="";stagedQuery=if(contact)null else query;stagedContact=if(contact)query else null;stagedPreview=preview;change("")}
            ...
            Column(Modifier.fillMaxWidth().heightIn(max=240.dp).verticalScroll(rememberScrollState()).padding(horizontal=16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)) {
                    if(hasStructured && panel.isEmpty()) {
                        ...
                        stagedPreview?.let {BuilderPreview(it)} ?: Text(if(stagedKind=="Randomizer")"The result is chosen when you send." else "Ready to send",style=MaterialTheme.typography.bodySmall)
                    }
            ...
            ComposerBar {
                ...
                Column(Modifier.weight(1f), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Composer(draft, analyze, Modifier.fillMaxWidth(), showTools = false, focusRequester = editor, namedFormatting = !hasAttachment && !editingCaption, showSource = showSource, onFocus = { if (panel == "Voice") command("record_stop", emptyMap()); if (panel in listOf("Attachments","Format")) {keyboardPending=true;panel=""} })
                }
                ...
                FilledIconButton({ if (hasAttachment) {
                        ...
                        if(stagedQuery!=null || stagedContact!=null) {pendingBuilder="$peer:$stagedKind" to sentCaption;command("post",attachmentTarget+mapOf("text" to sentCaption,"service_query" to stagedQuery,"shared_contact" to stagedContact,"rich" to true))} else {pendingBuilder="$peer:$stagedKind" to text;send(text,stagedRich,stagedTimezone)}
                    ...
                    Modifier.size(48.dp), enabled = if (hasAttachment) enabled && voice.phase != "Sending" && attachmentDrafts.none { it.phase == "Staging" } else if(hasStructured)enabled else if(helpQuery!=null)true else if (hasText) enabled || requestContact != null else LocalClientFeatures.current.voice, shape = RoundedCornerShape(16.dp),
...
internal fun StructuredBuilder(kind: String, enabled: Boolean, back: () -> Unit, send: (String, String?) -> Unit) {
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(horizontal = 24.dp, vertical = 12.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
        ...
        }, enabled = enabled && (!temporal || preview!=null && !checkingTime) && (if (kind == "Timer") whenText.isNotBlank() else title.isNotBlank()) && (kind !in listOf("Poll", "Checklist", "Task") || entries.count { it.isNotBlank() } >= if (kind == "Poll") 2 else 1), modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(16.dp)) { Text(LocalBuilderAction.current) }
    }
}
@Composable
...
private fun VoicePanel(command: Command, peer: String, voice: VoiceState, close: () -> Unit) {
    ...
    Column(Modifier.fillMaxSize().padding(horizontal=24.dp,vertical=16.dp),verticalArrangement=Arrangement.spacedBy(16.dp)) {
        Row(Modifier.fillMaxWidth(),verticalAlignment=Alignment.CenterVertically,horizontalArrangement=Arrangement.spacedBy(10.dp)) {
            ...
            Text(if(recording)"${voice.seconds/60}:${(voice.seconds%60).toString().padStart(2,'0')}" else "Voice message",Modifier.weight(1f),style=MaterialTheme.typography.titleMedium)
            if(recording)Text(if(voice.paused)"Paused" else "Recording",style=MaterialTheme.typography.labelLarge,color=MaterialTheme.colorScheme.onSurfaceVariant)
        }
        Box(Modifier.weight(1f).fillMaxWidth(),contentAlignment=Alignment.Center) {
            AudioWaveform(if(recording)voice.levels else List(40){.06f},Modifier.fillMaxWidth().heightIn(min=40.dp,max=88.dp))
        }
        Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.SpaceEvenly) {
            ComposerTool("Discard","delete",action=close)
            if(recording)ComposerTool(if(voice.paused)"Resume recording" else "Pause recording",if(voice.paused)"mic" else "pause") {command("record_pause",emptyMap())}
            ComposerTool(if(recording)"Use recording" else "Record",if(recording)"check" else "mic",primary=true) {command(if(recording)"record_stop" else "record_start",mapOf("peer" to peer))}
        }
    }
}
