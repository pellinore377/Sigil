...
internal fun toolGridHeight(items:List<Pair<String,String>>,width:Dp):Dp {
    ...
    val cell=with(density){((width-24.dp)/4).roundToPx().coerceAtLeast(1)}
    ...
    return rows.fold(0.dp) {height,row->
        ...
        height+64.dp+with(density){label.toDp()}
...
internal fun ComposerTool(name:String,icon:String,enabled:Boolean=true,primary:Boolean=false,action:()->Unit) {
    ...
    Column(Modifier.clip(RoundedCornerShape(12.dp)).clickable(enabled=enabled,role=Role.Button,onClick=action).semantics {contentDescription=name}.padding(vertical=2.dp),
        ...
        Text(name,style=MaterialTheme.typography.labelMedium,textAlign=TextAlign.Center,color=MaterialTheme.colorScheme.onSurface.copy(alpha=if(enabled)1f else .38f))
    }
}
...
internal fun AttachmentTools(hasAttachment:Boolean,hasStructured:Boolean,open:(String)->Unit) {
    ...
    BoxWithConstraints(Modifier.widthIn(max=660.dp).fillMaxWidth()) {
        ...
        LazyVerticalGrid(GridCells.Fixed(4),modifier=Modifier.fillMaxWidth().height(height),contentPadding=PaddingValues(start=8.dp,end=8.dp,top=8.dp),verticalArrangement=Arrangement.spacedBy(4.dp),horizontalArrangement=Arrangement.spacedBy(8.dp)) {
