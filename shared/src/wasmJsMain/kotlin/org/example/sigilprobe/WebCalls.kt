            ...
            try {
                ...
                maintenance=scope.launch {
                    while(isActive && current==generation && desired==call.id){
                        ...
                        browserTimingLog("SigilTiming call transport=$state receivers=$receivers members=${members.size} audio=$audio"+(refreshed.exceptionOrNull()?.let{" refresh_error=${it.message?.take(80)}"}?:""))
