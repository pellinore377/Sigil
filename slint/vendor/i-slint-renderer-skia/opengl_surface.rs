...
impl super::Surface for OpenGLSurface {
    fn new(
        ...
        Self::new_with_config(
            ...
            // SIGIL PATCH: an 8-bit stencil buffer. Without one, Skia has no
            // way to clip a rounded rectangle whose corners differ (a grouped
            // message bubble: 16 on the outer corners, 5 on the inner) on the
            // GPU, and rasterises a software coverage mask the size of the
            // clip for every card, every frame — measured on the phone as
            // memset and mask upload in GrSWMaskHelper, several milliseconds
            // a frame with a few picture cards on screen. With a stencil
            // buffer the same clips go through the GPU's stencil.
            glutin::config::ConfigTemplateBuilder::new().with_stencil_size(8),
            None,
        )
    }
