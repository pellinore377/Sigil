        ...
        match event {
            ...
            MouseEvent::Moved { position, .. } => {
                ...
                if do_intercept {
                    ...
                    // SIGIL PATCH (with the input.rs delayed-press patch): a
                    // sub-threshold move during a press must not transfer the
                    // grab — the child whose press timer is running would get
                    // Exit, so a finger's natural jitter cancels every
                    // long-press. The flick still starts: the move that
                    // crosses DISTANCE_THRESHOLD lands in the Intercept arm.
                    let stationary = inner
                        .pressed_mouse_state
                        .is_some_and(|(_, pressed_pos)| {
                            let d = *position - pressed_pos;
                            abs(d.x_length()) <= DISTANCE_THRESHOLD
                                && abs(d.y_length()) <= DISTANCE_THRESHOLD
                        });
                    if stationary {
                        InputEventFilterResult::ForwardEvent
                    } else {
