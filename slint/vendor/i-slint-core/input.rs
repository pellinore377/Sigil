...
pub fn process_mouse_input(
    ...
    if mouse_input_state.delayed.is_some()
        && (!accepted
            // SIGIL PATCH (upstream: Flickable press-delay vs touch): while a
            // press is delay-pending, an accepted Moved must not replace the
            // input state — dropping it cancels the delayed-forward timer, so
            // on touch (which streams stationary moves) children never receive
            // the press and long-press is impossible. Keep the pending state
            // for moves; big moves still flick once the filter intercepts on a
            // later event, and the timer flushes the press at the delay's end.
