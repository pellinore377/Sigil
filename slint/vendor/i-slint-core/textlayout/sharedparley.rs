...
impl FontContext {
    ...
    pub fn register_static_font(&mut self, data: &'static [u8]) {
        ...
        if self.registered_static_fonts.insert(key) {
            ...
            // SIGIL PATCH: a registered emoji font joins the Emoji generic
            // family, so emoji clusters in ANY text (parley appends
            // GenericFamily::Emoji for them) fall back to it. Android's system
            // emoji font freezes with the OS release and misses newer Unicode.
            for (family_id, _) in &fonts {
                let is_emoji = self
                    .inner
                    .collection
                    .family_name(*family_id)
                    .is_some_and(|n| n.contains("Emoji"));
                if is_emoji {
                    self.inner.collection.append_generic_families(
                        fontique::GenericFamily::Emoji,
                        core::iter::once(*family_id),
                    );
                }
            }
        }
