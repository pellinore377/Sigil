...
impl JavaHelper {
    ...
    pub fn set_imm_data(
        ...
        self.with_jni_env(|env, helper| {
            ...
            // SIGIL PATCH: a single-line field (a server name, a username, an
            // invite code) must not be autocorrected — Gboard rewrote
            // "example-name.com" while it was typed. The composer is
            // multi-line and keeps suggestions.
            let no_suggest = if single_line { 0x0008_0000 } else { 0 };
            let input_type = no_suggest | match data.input_type {
    ...
    pub fn set_dark_system_bars(&self, dark: bool) -> Result<(), jni::errors::Error> {
        self.with_jni_env(|env, helper| helper.set_dark_system_bars(env, dark as jboolean))
    }
