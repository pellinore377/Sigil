            ...
            InputMethodRequest::Enable(props) => {
                self.java_helper
                    .set_imm_data(
                        ...
                        focused_single_line(&self.window),
                    )
...
/// Whether the focused item is a single-line TextInput (SIGIL PATCH, see
/// set_imm_data: those get IME suggestions turned off).
fn focused_single_line(window: &i_slint_core::api::Window) -> bool {
    i_slint_core::window::WindowInner::from_pub(window)
        .focus_item
        .borrow()
        .upgrade()
        .and_then(|item| {
            item.downcast::<i_slint_core::items::TextInput>()
                .map(|ti| ti.as_pin_ref().single_line())
        })
        .unwrap_or(false)
}
