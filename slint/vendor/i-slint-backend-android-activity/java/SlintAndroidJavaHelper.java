...
public class SlintAndroidJavaHelper {
    ...
    public SlintAndroidJavaHelper(Activity activity) {
        ...
        this.mActivity.runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                // SIGIL PATCH: edge-to-edge — draw behind the status and gesture
                // bars; the app applies the safe-area insets itself.
    ...
    // SIGIL PATCH: light system-bar glyphs over a dark app and dark glyphs
    // over a light one.
    public void set_dark_system_bars(final boolean dark) {
        mActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                    android.view.WindowInsetsController c = mActivity.getWindow().getInsetsController();
                    if (c != null) {
