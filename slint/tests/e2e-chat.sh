...
# A port of this run's own: two suites on one fixed port answer each other's
# invites with 401s and look like a broken server.
...
XDG_STATE_HOME=$W/state XDG_CACHE_HOME=$W/cache HOME=$W DRIVE_SYNC=$W/sync \
  timeout 480 "$DRIVE" "$W/shots" 127.0.0.1:$PORT "$IB" bob chat >drive.out 2>drive.err & PIDS+=($!)
DRIVE_PID=$!
