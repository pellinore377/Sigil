...
fuzz_target!(|bytes: &[u8]| {
    if bytes.len() < 33 || bytes.len() > 32801 {
        return;
    }
    ...
    match bytes[0] {
        0 => {
            if let Ok(session) =
                sigil_crypto::triple::Session::open_checkpoint(&key, &sealed, b"Sigil/fuzz-only")
            {
                let next = session.seal_checkpoint(&key, b"Sigil/fuzz-only").unwrap();
                let restored =
                    sigil_crypto::triple::Session::open_checkpoint(&key, &next, b"Sigil/fuzz-only")
                        .unwrap();
        ...
        1 => {
            if let Ok(session) =
                sigil_crypto::ratchet::Session::open_checkpoint(&key, &sealed, b"Sigil/fuzz-only")
            {
                let next = session.seal_checkpoint(&key, b"Sigil/fuzz-only").unwrap();
                assert!(sigil_crypto::ratchet::Session::open_checkpoint(
                    &key,
                    &next,
                    b"Sigil/fuzz-only"
                )
        ...
        2 => {
            if let Ok(slot) = sigil_crypto::handshake::Receiver::open_checkpoint(
                &key,
                &sealed,
                b"Sigil/fuzz-only",
                identity,
                ...
                assert!(sigil_crypto::handshake::Receiver::open_checkpoint(
                    &key,
                    &next,
                    b"Sigil/fuzz-only",
                    identity
        ...
        3 => {
            if let Ok(identity) =
                sigil_crypto::IdentityKey::open_checkpoint(&key, &sealed, b"Sigil/fuzz-only")
            {
                let next = identity.seal_checkpoint(&key, b"Sigil/fuzz-only").unwrap();
                assert_eq!(
                    sigil_crypto::IdentityKey::open_checkpoint(&key, &next, b"Sigil/fuzz-only")
                        .unwrap()
                        .public_key(),
                    identity.public_key()
                );
