...
pub(crate) fn admin() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/admin/v0/push/android",
            get(android_configuration).put(configure_android),
        )
...
pub(crate) fn client() -> Router<AppState> {
    Router::new()
        ...
        .route("/client/v0/push/android", get(android_provider))
...
async fn android_configuration(State(state): State<AppState>) -> Response {
    match with_store(state, |store| store.push_android_configuration()).await {
        Ok(v) => Json(v).into_response(),
        Err(e) => store_error(e),
    }
}
async fn configure_android(
    State(state): State<AppState>,
    Json(request): Json<crate::push_android::Configure>,
) -> Response {
    match with_store(state, move |store| store.configure_push_android(request)).await {
        Ok(v) => Json(v).into_response(),
        Err(e) => store_error(e),
    }
}
async fn android_provider(State(state): State<AppState>, headers: HeaderMap) -> Response {
    let credential = match bearer(&headers) {
        Ok(v) => v,
        Err(e) => return store_error(e),
    };
    match with_store(state, move |store| store.push_android(&credential, now()?)).await {
        Ok(v) => Json(v).into_response(),
        Err(e) => store_error(e),
    }
}
