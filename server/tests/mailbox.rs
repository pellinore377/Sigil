...
fn failed_reciprocal_grant_does_not_consume_invite_or_leave_one_way_permission() {
    ...
    store
        ...
        .unwrap();
    let db = rusqlite::Connection::open(&path).unwrap();
    db.execute("WITH RECURSIVE n(x) AS (VALUES(1) UNION ALL SELECT x+1 FROM n WHERE x<256) INSERT INTO devices(id,account_id,label,expires_at) SELECT printf('%064x',x),?1,'Synthetic',999999 FROM n",[owner.account_id]).unwrap();
